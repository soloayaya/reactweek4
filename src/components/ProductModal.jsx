...
function ProductModal({
  ...
  const replaceText = (message) => {
    if (typeof message !== "string") {
      return message; // 確保 message 是字串
    }
    return message
      .replace(/title/g, "標題")
      .replace(/category/g, "分類")
      .replace(/unit/g, "單位")
      .replace(/ 屬性/g, ""); // 移除「 屬性」這個詞
  };
  ...
  const createProduct = async () => {
    try {
      ...
      const formattedMessage = Array.isArray(errorMessage)
        ? errorMessage.map(replaceText).join(", ") // 逐一轉換並用逗號分隔
        : replaceText(errorMessage); // 直接轉換

      alert(`新增產品失敗: ${formattedMessage || "未知錯誤"}`);
    }
  };
